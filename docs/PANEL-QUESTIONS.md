...
|---|---|---|---|---|
| A1 | ~~**Tunnels API does not respond**~~ — `list` **fixed 2026-08-26**; two per-tunnel routes still 404 | 2 of 7 | Partly resolved | Confirm the intended path shape |
...
## A1. The Tunnels API — mostly fixed, two routes still 404

> **Re-probed 2026-08-26.** `GET /v0/tunnels/list` now works: **HTTP 200, 266 tunnels over
> 2 pages** (200 per page — note that is *not* the 100 `locations/list` uses), no duplicate
> `tunnel_id`s, 0.31 s through our own client. `tunnelsAvailable()` now returns `true`, so
> everything gated behind it is live.
>
> Two routes are still unreachable, and it is a **route-shape** problem rather than a missing
> record:
>
> ```
> GET /v0/tunnels/818867/class/TunnelBroker       → 404 (HTML)
> GET /v0/tunnels/info/818867/class/TunnelBroker  → 404 (HTML)
> GET /v0/tunnels/818867                          → 200 {"status":"ok","tunnel_id":"818867",
>                                                        "local_ip":…, "remote_ip":…,
>                                                        "network48":…, "network64":…}
> ```
>
> Probed with a real `tunnel_id` + `class` taken straight out of `list`, for one tunnel of
> each of the three classes present (TunnelBroker, RouteGre, NewRoute) — all three 404 with
> the `/class/{class}` segment and all three answer without it. So the panel appears to have
> **dropped `/class/{class}`**, and the surviving `/tunnels/{id}` returns the *provider-info*
> body that `tunnels/info/...` is documented to return.
>
> **Worked around on our side — no longer blocking.** Both methods try the documented path
> first and fall back only on a 404, so they revert to a single direct call the moment the
> routes come back, with no code change:
>
> | method | documented path | fallback |
> |---|---|---|
> | `tunnelInfo()` | `/tunnels/info/{id}/class/{class}` | `GET /tunnels/{id}` |
> | `tunnel()` | `/tunnels/{id}/class/{class}` | the row `list` already returns, re-wrapped as `{status, tunnels:[row]}` |
>
> `tunnel()` cannot use `/tunnels/{id}`: that returns the provider view, which has none of
> `location_id`, `service_id`, `email`, `username`, `status` or `tag`. `list` carries exactly
> the documented row, so it is the same data from the same source.
>
> **Question 4 (still open):** is dropping `/class/{class}` intended? If yes, `tunnels.md`
> needs correcting and we will delete the fallbacks. If no, the two routes are still missing.
>
> **Question 5 (new): `tunnels/info` fails for about a third of `NewRoute` tunnels.** Not a
> routing problem — the panel answers `200` with its own error body:
>
> ```json
> {"status":"error","description":"Unable to get tunnel info: 404|"}
> ```
>
> Sampled 12 tunnels per class: TunnelBroker 2/2 ok, RouteGre 4/4 ok, **NewRoute 8/12 ok, 4/12
> failing**. NewRoute is 260 of the 266 tunnels, so this is the common case. The trailing `|`
> in the message suggests a truncated upstream error. Is the provider rejecting these, or is
> the panel's stored credential/id for them stale?
>
> **Also unverified:** `new`, `update`, `delete` and `status` are all documented with the same
> `/class/{class}` segment as the two 404s, so treat them as suspect. We have not probed them
> because they mutate real infrastructure — please confirm their current paths.
>
...
- ~~**`total_pages` is wrong.**~~ **Fixed 2026-08-26** — now reports `total_pages: 3` for the
  246 locations, matching `ceil(246/100)`. It previously reported `2` while page 3 returned
  the missing 46, so anyone trusting it silently lost a fifth of the catalogue. We still page
  until an empty batch rather than trusting the count: it costs one extra request, it was
  wrong once, and page 4 returning `rows: 0` (rather than a 404) makes it safe.
