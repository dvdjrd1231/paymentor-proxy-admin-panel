...
| **Blank page / CSS-JS 404 / broken Login-Register links** (port-less URLs like `http://localhost/...`) | Generated links + asset URLs must match the URL you open. Set `APP_URL` **and** `ASSET_URL` to the exact scheme+host+port (e.g. `https://billing.example.com`), then `php artisan config:cache`. Serving on a non-standard host port (e.g. `:8080`) can drop the port on the nginx→php-fpm hop, producing port-less URLs; the local Docker stack avoids this by serving on the standard port 80 (`http://localhost`). |
